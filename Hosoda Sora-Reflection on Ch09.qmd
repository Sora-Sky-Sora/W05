...
1.  **How many customers actually responded ("yes") and were correctly predicted to respond? What is this cell called?**
    ...
    > No customers were correctly predicted as responders. The cell for correctly predicted responders is called true positive (TP).

2.  **How many customers actually responded but were predicted "no"? What is this cell called, and what does it cost a marketer**

    > Thirty-one customers responded but were classified as "no." These are false negatives (FN). Missing these customers means losing potential sales because they would not receive the marketing campaign.

3.  **How many customers did not respond but were predicted "yes"? What is this cell called, and what does it cost a marketer**

    > There were no false positives (FP) in this confusion matrix. If false positives occurred, the company would spend money sending mail to customers who would not make a purchase.
...
4.  **In plain English, which type of error is more costly for a direct-mail campaign with a \$5 mailing cost and \$50 average order value — a false positive or a false negative? Explain your reasoning.**
    ...
    > A false negative is more costly in this case. Sending an unnecessary mail costs about \$5, but failing to contact a customer who would have responded could lose an average order worth about \$50.
...
> Neither precision nor sensitivity increased after changing the threshold from 0.5 to 0.3. The largest predicted probability was only about 0.071, so every customer was still classified as "no." Sensitivity stayed at 0 and precision remained `NA`. A lower threshold often increases sensitivity because more customers are classified as responders. In this dataset, however, every predicted probability was below 0.3, so the threshold change had no effect.
...
1.  **Look up the AUC value from the `roc-curve` chunk. Is it above or below 0.65 (the "usable for targeting" threshold from the callout)?**
    ...
    > The AUC is 0.473, which is below the suggested threshold of 0.65.
...
2.  **In plain English, what does an AUC of that value mean? Complete this sentence: "If we randomly pick one customer who responded and one who did not, the model assigns a higher predicted probability to the responder \_\_\_\_% of the time."**
    ...
    > If we randomly pick one customer who responded and one who did not, the model assigns a higher predicted probability to the responder about 47.3% of the time.
...
3.  **The ROC curve plot shows the sensitivity/specificity tradeoff across all thresholds. At what point on the curve would you operate if your campaign budget only allows you to mail the top 10% of prospects?**
    ...
    > A point with high specificity is more suitable when only a small number of customers can be contacted. Focusing on customers with the highest predicted probabilities helps keep mailing costs under control.
...
> ROC AUC and sensitivity are the most informative measures for this model. Accuracy alone is less useful because the response rate is low. ROC AUC summarizes performance across different classification thresholds, while sensitivity shows how many actual responders are identified. Both measures are useful when evaluating a direct-mail campaign.

```{r ex9-2b}
marketing_metrics <- metric_set(accuracy, roc_auc, sens, spec, f_meas)
...
> Adding one whale customer changed RMSE much more than MAE. One large prediction error affected RMSE strongly because large errors receive more weight. MAE also increased, but much less. When a few customers place unusually large orders, MAE changes less than RMSE and is easier to interpret.
...
1.  **Why might micro-averaging look more favorable when one class (Bronze) is much larger than the others?**
    ...
    > Micro-averaging gives more weight to larger classes. Since Bronze has many more customers than the other loyalty tiers, its performance has a stronger influence on the overall result.
...
2.  **Which estimator would you choose if you wanted to ensure that Platinum customers — the rarest and most valuable tier — are not underrepresented in the metric? Explain your reasoning.**
    ...
    > Macro averaging is a better choice because every class is treated equally. This allows the Platinum tier to contribute as much as the larger groups, even though it has fewer customers.
...
3.  **In a marketing context, which averaging method is most likely to give a misleadingly optimistic picture of model performance? Why?**
    ...
    > Micro-averaging can give a more optimistic result because the largest customer group has the strongest influence on the metric. Strong performance for Bronze customers can make the overall result look better even when smaller groups are predicted less accurately.
...
<https://sora-sky-sora.github.io/W05/Hosoda%20Sora-Reflection%20on%20Ch09.html>

### GitHub Repository

<https://github.com/Sora-Sky-Sora/W05>